...
export const AuthProvider = ({ children }) => {
  ...
  const fetchUserRole = useCallback(async (userId) => {
    if (!isMounted) {
      authDebug('fetchUserRole: componente não montado');
      return;
    }
  ...
  const checkUser = useCallback(async () => {
    try {
      authDebug(' Verificando sessão inicial...');
      
      const { data: { session }, error } = await supabase.auth.getSession();
      authDebug('Resposta de getSession:', { hasSession: !!session, email: session?.user?.email, error });
      
      if (!isMounted) {
        authDebug('Componente desmontado após getSession, ignorando setUser');
        return;
      }
      ...
      if (session?.user) {
        authDebug('Usuário encontrado:', session.user.id);
        ...
        authDebug('Nenhuma sessão ativa na inicialização');
        setUser(null);
        setRole('user');
        setRoleLoading(false);
      }
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (!isMounted) {
          authDebug('Componente desmontado, ignorando mudança de auth');
          return;
        }
